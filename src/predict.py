...
def predict_next_close():
    ...
    try:
        ...
        # Added compile=False to avoid metric deserialization errors
        model = tf.keras.models.load_model("models/nifty50_return_lstm.h5", compile=False)
        ...
        # Handle VIX alignment
        df['VIX_Close'] = vix['Close'].reindex(df.index).ffill().fillna(15.0)
        df = df.reset_index()
        df = engineer_features(df).dropna()
        ...
        try:
            ...
            if isinstance(last, pd.Series):
                 last = last.iloc[0]
            print(f"Falling back to latest close: ₹{float(last):,.2f}")
